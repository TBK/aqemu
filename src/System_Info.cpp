...
VM::Emulator_Version System_Info::Get_Emulator_Version(const QString &path) {
  if (path.isEmpty()) {
    AQWarning("VM::Emulator_Version System_Info::Get_Emulator_Version( const "
              "QString &path )",
              "path is Empty");
    return VM::Obsolete;
  }
  ...
  for (int ix = 0; ix < 5; ++ix) {
    ...
    if (tmpLine.isEmpty()) {
      AQError("VM::Emulator_Version System_Info::Get_Emulator_Version( const "
              "QString &path )",
...
System_Info::Find_QEMU_Binary_Files(const QString &path) {
  ...
  if (!QFile::exists(path)) {
    AQError("QMap<QString, QString> System_Info::Find_QEMU_Binary_Files( const "
            "QString &path )",
  ...
  /*// Next code for QEMU 1.0 in it version 'qemu' binary name changed to
'qemu-system-i386' if( emulFiles["qemu"].isEmpty() )
{
  #ifdef Q_OS_WIN32
  if( QFile::exists(dirPath + "qemu-system-i386.exe") ) emulFiles[ "qemu" ] =
dirPath + "qemu-system-i386.exe"; #else if( QFile::exists(dirPath +
"qemu-system-i386") ) emulFiles[ "qemu" ] = dirPath + "qemu-system-i386"; #endif
}*/
...
{
    AQError( "QMap<QString, QString> System_Info::Find_QEMU_Binary_Files( const
QString &path )", QString("Path \"%1\" not exists!").arg(path) ); return
emulFiles;
}

// Find
QString dirPath = QDir::toNativeSeparators( (path.endsWith("/") ||
path.endsWith("\\")) ? path : path + "/" );

if( QFile::exists(dirPath + "kvm") ) emulFiles[ "qemu-kvm" ] = dirPath + "kvm";
else if( QFile::exists(dirPath + "qemu-kvm") ) emulFiles[ "qemu-kvm" ] = dirPath
+ "qemu-kvm";
...
Available_Devices System_Info::Get_Emulator_Info(const QString &path, bool *ok,
  ...
  if (!QFile::exists(path)) {
    AQError("Available_Devices System_Info::Get_Emulator_Info( const QString "
            "&path, bool *ok,"
    ...
    /*for ( int i = 0; i < rx_list.count(); i++)
{
  ...
  // -net socket
  rx = QRegExp(".*(-net(?:dev)?\\s+socket\\,.*)-net.*");
  if (rx.exactMatch(all_help)) {
    QStringList rx_list = rx.capturedTexts();
    if (rx_list.count() > 1)
      net_str += rx_list[1];
    ...
    for (int lx = 0; lx < regExpVariants.count(); ++lx) {
      ...
      if (tmp_rx.exactMatch(tmp)) {
        ...
        if (rx_list.count() > 1) {
          ...
          AQError("Available_Devices System_Info::Get_Emulator_Info( const "
  ...
  do {
    ...
    if (!(dev_map.QEMU_Name.isEmpty() ||
          dev_map.QEMU_Name.indexOf(QRegExp("/^\\S+$/"), 0) != -1)) {
  ...
  if (rx.exactMatch(vga_list_string)) {
    ...
    if (rx.capturedTexts().count() > 1) {
      QStringList vga_devices_list =
          rx.capturedTexts()[1].split("|", QString::SkipEmptyParts);
  ...
  do {
    ...
    if (tmp.isEmpty() || tmp.startsWith("Valid sound card names") ||
        tmp.startsWith("-soundhw") || tmp.indexOf(QRegExp("/^\\S+$/"), 0) != -1)
      continue;
    ...
    if (qemu_dev_name == "sb16")
      ...
      if (!qemu_dev_name.isEmpty())
        AQWarning("Available_Devices System_Info::Get_Emulator_Info( const "
                  "QString &path, bool *ok,"
                  "VM::Emulator_Version version, const QString &internalName )",
  ...
  do {
    ...
    QString all_models = tmp.mid(
        tmp.indexOf("models:") + QString("models:").count(), tmp.count());
    all_models.remove(QChar(' ')); // Remove all spaces

    QStringList net_cards_models =
        all_models.split(',', QString::SkipEmptyParts);
  ...
  // Return info
  AQDebug("Available_Devices System_Info::Get_Emulator_Info( const QString "
          "&path, bool *ok,"
          "VM::Emulator_Version version, const QString &internalName )",
          "Done");
  *ok = true;
...
QString System_Info::Get_Emulator_Help_Output(const QString &path) {
  ...
  if (result.isEmpty()) {
    ...
    if (result.isEmpty()) {
      AQError("QString System_Info::Get_Emulator_Help_Output( const QString "
              "&path )",
...
QString System_Info::Get_Emulator_Output(const QString &path,
  ...
  if (!qemu_pr->waitForFinished(2000)) {
    AQError("QStringList System_Info::Get_Emulator_Output( const QString "
            "&path, const QStringList &args )",
...
const QList<VM_USB> &System_Info::Get_Used_USB_List() { return Used_Host_USB; }
...
bool System_Info::Read_SysFS_File(const QString &path, QString &data) {
  ...
  if (!sysfs_file.open(QIODevice::ReadOnly | QIODevice::Text)) {
    AQWarning("bool System_Info::Read_SysFS_File( const QString &path, QString "
              "&data )",
...
bool System_Info::Scan_USB_Proc(QList<VM_USB> &list) {
  ...
  QRegExp busAddr = QRegExp(
      "T:[\\s]+Bus=([\\d]{2}).+Dev#=[\\s]*([\\d]+).+Spd=([\\d]{1,3}).*");

  // P:  Vendor=0123 ProdID=abcd Rev= 0.01
  QRegExp idHex =
      QRegExp("P:[\\s]+Vendor=([\\dabcdef]{4})[\\s]+ProdID=([\\dabcdef]{4}).+");
    ...
    for (int bx = 0; bx < linux_usb_dev[ix].count(); bx++) {
      ...
      if (manufacturer_list.count() <= 0) {
        if (manufacturer.exactMatch(linux_usb_dev[ix][bx])) {
          ...
          AQError("bool System_Info::Scan_USB_Proc( QList<VM_USB> &list )",
                  "Cannot Match manufacturer! String: " +
                      linux_usb_dev[ix][bx]);
        }
      }
      ...
      if (serialNumber_list.count() <= 0) {
        if (serialNumber.exactMatch(linux_usb_dev[ix][bx])) {
          ...
          AQError("bool System_Info::Scan_USB_Proc( QList<VM_USB> &list )",
                  "Cannot Match serialNumber! String: " +
                      linux_usb_dev[ix][bx]);
        }
      }
    }
...
QStringList System_Info::Get_Host_CDROM_List() {
  ...
  for (int ix = 0; ix < cdrom_count; ix++) {
    tmp_list << settings
                    .value("Additional_CDROM_Devices/Device" +
                               QString::number(ix),
                           "")
                    .toString();
  }

...
void AQEMU_Main::first_start_wizard() {
  ...
  if (settings->value("First_Start", "yes").toString() == "yes") {
    First_Start_Wizard first_start_win(nullptr);
...
void AQEMU_Main::register_qresource() {
  ...
  iconsThemeFile = QDir::toNativeSeparators(
  settings->value("AQEMU_Data_Folder", "").toString() + "/icons.rcc" );

  if( ! QResource::registerResource(iconsThemeFile) )
  {
      AQGraphic_Error( "int main( int argc, char *argv[] )",
  QObject::tr("Error!"), QObject::tr("Cannot Load AQEMU Icon Theme!\nFile \"%1\"
  Not Found!").arg(iconsThemeFile), false );
  }*/
}
  ...
  if (QFile::exists(settings->fileName())) {
    ...
    if (conf_ver == "0.5") {
      ...
      QMessageBox::information(
          nullptr, QObject::tr("AQEMU emulators search"),
          QObject::tr(
              "AQEMU will search for emulators after uptating. Please wait."),
          QMessageBox::Ok);

      First_Start_Wizard *first_start_win = new First_Start_Wizard(nullptr);
  ...
  if (settings->value("AQEMU_Data_Folder", "").toString().isEmpty()) {
    ...
    if (settings->value("AQEMU_Data_Folder", "").toString().isEmpty()) {
      ...
      QString aqemuDataDir = QFileDialog::getExistingDirectory(
          nullptr, QObject::tr("Please Select AQEMU Data Folder:"), "/",
...
int main(int argc, char *argv[]) { return AQEMU_Main().main(argc, argv); }

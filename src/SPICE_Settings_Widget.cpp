...
SPICE_Settings_Widget::SPICE_Settings_Widget(QWidget *parent)
  ...
  // connect( ui.SB_QXL_Num, SIGNAL(valueChanged(int)), this,
...
const VM_SPICE &SPICE_Settings_Widget::Get_Settings(bool &settingsValidated) {
  ...
  /*unsigned int val = ui.CB_RAM_Size->currentText().toUInt( &ok );
     ...
     ); settingsValidated = false; return spiceSettings;
      }*/

  val = ui.Edit_SPICE_Port->value();

  spiceSettings.Set_Port(val);
  /*if( val > 0 ) spiceSettings.Set_Port( val ); //TODO: seperate validation and
     / get settings else
      {
              AQGraphic_Warning( tr("Error"),
                                                 tr("SPICE port number
     incorrect!") ); settingsValidated = false; return spiceSettings;
      }*/
  ...
  /*if( val > 0 ) spiceSettings.Set_SPort( val ); //TODO: seperate validation
     and / get settings else
      {
              AQGraphic_Warning( tr("Error"),
                                                 tr("SPICE SPort number
     incorrect!") ); settingsValidated = false; return spiceSettings;
      }*/
...
void SPICE_Settings_Widget::Show_Renderer_List(
  ...
  for (int ix = 0; ix < list.count(); ++ix) {
    ...
    default:
      renderStr = tr("Error!");
      AQError("void SPICE_Settings_Widget::Show_Renderer_List( const "

...
void Snapshots_Window::on_Button_Create_clicked() {
  ...
  if (prop_win.exec() == QDialog::Accepted) {
    ...
    // Current_VM->Add_Snapshot( tag, prop_win.Get_Snapshot_Name(),
    // prop_win.Get_Snapshot_Description() ); Current_VM->Save_VM_State( tag,
    // false ); //TODO
...
void Snapshots_Window::on_Button_Delete_clicked() {
  if (ui.Snapshots_Tree->currentItem() == nullptr)
    return;
...
bool Snapshots_Window::Update_Info() {
  ...
  if (info_lines.count() == 2) {
    ...
    QRegExp snap_regexp = QRegExp("([\\d])+[\\s]+([a-zA-Z0-9_]+)[\\s]+([0-9KMG]"
                                  "+|[0-9]+.[0-9KMG]+)[\\s]+(.+)[\\s]{2,}(.+)");

    for (int sx = 0; sx < snapshots_list.count(); ++sx) {
      if (!snap_regexp.exactMatch(snapshots_list[sx]))
        continue;

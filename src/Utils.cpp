...
void AQGraphic_Warning(const QString &caption, const QString &mes) {
  ...
  QMessageBox::warning(nullptr, caption, mes, QMessageBox::Ok);
}
...
void AQGraphic_Warning(const QString &sender, const QString &caption,
  ...
  if (fatal) {
    ...
    QMessageBox::warning(
        nullptr, caption, QString("Sender: %1\nMessage: %2").arg(sender).arg(mes),
        QMessageBox::Ok);
  }
...
void AQUse_Log(bool use) { Save_Messages_To_Log = use; }
...
void AQLog_Path(const QString &path) { Log_Path = path; }
...
void AQSave_To_Log(const QString &mes_type, const QString &sender,
  ...
  if (!log_file.open(QIODevice::Append | QIODevice::Text)) {
    ...
    AQError("void AQSave_To_Log( const QString& mes_type, const QString& "
            "sender, const QString& mes )",
...
bool Create_New_HDD_Image(bool encrypted, const QString &base_image,
  ...
  if (!qemu_img.waitForFinished(10000)) {
    ...
    if (err.count() > 0) {
      AQGraphic_Error("bool Create_New_HDD_Image( bool encrypted, const "
                      "QString &base_image,"
                      "const QString &file_name, const QString &format, "
                      "VM::Device_Size size, bool verbose )",
                      QObject::tr("Error!"),
...
const QStringList &Get_CD_Recent_Images_List() { return Recent_CD_Images; }
...
const QStringList &Get_FDD_Recent_Images_List() { return Recent_FDD_Images; }
...
bool Get_Show_Error_Window() { return Show_Error_Window; }

void Set_Show_Error_Window(bool show) { Show_Error_Window = show; }

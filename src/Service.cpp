...
AQEMU_Service::~AQEMU_Service() { delete service; }

void AQEMU_Service::setMainWindow(bool b) { main_window = b; }

void AQEMU_Service::setMain(AQEMU_Main *m) { main = m; }

int AQEMU_Service::machineCount() const { return machines.count(); }
...
void AQEMU_Service::vm_state_changed(Virtual_Machine *vm, VM::VM_State s) {
  ...
  // remove VM from service and shutdown the service, if there are no VMs
  // running
  if (s == VM::VMS_Power_Off) {
    machines.removeAll(vm);
    // delete vm; //Segfaults //Destructor was already called at this point
    // //Investigate how/why
  }
  if (machineCount() < 1) {
    if (!main_window) {
...
bool AQEMU_Service::isActive() { return (machines.count() > 0); }

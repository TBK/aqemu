...
MachineView::MachineView(QWidget *parent, Virtual_Machine *cur_vm)
    : QScrollArea(parent) {
...
void MachineView::Set_Scaling(bool s) {
  ...
  if (Scaling)
    newViewSize(maximumViewportSize().width(), maximumViewportSize().height());
  else
    newViewSize(-1, -1);
}

void MachineView::Set_Fullscreen(bool on) { fullscreen(on); }
...
void MachineView::connectView() {
  connect(View, SIGNAL(connected()), this, SIGNAL(Connected()));

  connect(View, SIGNAL(connected()), this, SLOT(VNC_Connected_OK()));

  connect(View, SIGNAL(framebufferSizeChanged(int, int)), this,
          SLOT(newViewSize(int, int)));

  connect(View, SIGNAL(MouseEnteredFromTheLeft()), this,
          SLOT(on_MouseEnteredFromTheLeft()));
  connect(View, SIGNAL(MouseEnteredFromTheRight()), this,
          SLOT(on_MouseEnteredFromTheRight()));
  connect(View, SIGNAL(MouseEnteredFromTheTop()), this,
          SLOT(on_MouseEnteredFromTheTop()));
  connect(View, SIGNAL(MouseEnteredFromTheBottom()), this,
          SLOT(on_MouseEnteredFromTheBottom()));
}

void MachineView::Check_Connection() {
  if (!VNC_Connected) {
    connect(Reinit_Timer, SIGNAL(timeout()), this,
            SLOT(initView())); // FIXME reinitVNC()
...
void MachineView::reinitVNC() {
  ...
  if (Init_Count < 10) {
    ...
    disconnect(View, SIGNAL(connected()), this, SIGNAL(Connected()));

    disconnect(View, SIGNAL(connected()), this, SLOT(VNC_Connected_OK()));

    disconnect(View, SIGNAL(framebufferSizeChanged(int, int)), this,
               SLOT(newViewSize(int, int)));
    /*
    View = new VncView( this );
    splashShown = true;
...
void MachineView::disconnectVNC() {
  ...
  disconnect(Reinit_Timer, SIGNAL(timeout()), this, SLOT(reinitVNC()));
...
void MachineView::captureAllMouseEvents() { View->grabMouse(); }

void MachineView::sendKey(QKeyEvent *event) { View->keyEvent(event); }

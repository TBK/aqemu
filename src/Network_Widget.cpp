...
bool Network_Widget::Get_Network_Cards(QList<VM_Net_Card_Native> &cards) {
  if (ui.Items_List->count() < 1) {
    AQDebug("bool Network_Widget::Get_Network_Card( QList<VM_Net_Card_Native> "
            "&cards ) const",
            "No Items");
    cards.clear();
    return true;
...
void Network_Widget::on_Items_List_currentItemChanged(
...
void Network_Widget::on_TB_Help_clicked() {
  ...
  if (ui.CB_Network_Type->currentText() == "nic")
    QMessageBox::information(
        ...
        tr("-net nic[,macaddr=addr][,model=type][,name=name] \nCreate a new "
           "Network Interface Card and connect it to Virtual LAN n (n = 0 is "
           "the default). The NIC is an ne2k_pci by default on the PC target. "
  ...
  // -net
  // tap[,name=name][,fd=h][,ifname=name][,script=file][,downscript=dfile][,helper=helper]
  ...
  // -net socket[,name=name][,fd=h][,listen=[host]:port][,connect=host:port]
    ...
    QMessageBox::information(
        ...
        tr("-net "
           ...
           "\nConnect the Virtual LAN n to a remote Virtual LAN in another "
           "QEMU virtual machine using a TCP socket connection. If \'listen\' "
           "is specified, QEMU waits for incoming connections on port (host is "
           "optional). \'connect\' is used to connect to another QEMU instance "
           "using the \'listen\' option. \'fd\'=h specifies an already opened "
           "TCP socket."));
  ...
  // -net socket[,name=name][,fd=h][,mcast=maddr:port]
    ...
    QMessageBox::information(
        ...
        tr("-net socket[,name=name][,fd=h][,mcast=maddr:port] \nCreate a "
           "Virtual LAN n shared with another QEMU virtual machines using a "
           "UDP multicast socket, effectively making a bus for every QEMU with "
           "same multicast address maddr and port. \nNOTES: \n1. Several QEMU "
           "can be running on different hosts and share same bus (assuming "
  ...
  // vde[,name=name][,sock=socketpath][,port=n][,group=groupname][,mode=octalmode]
    ...
    QMessageBox::information(
        ...
        tr("-net "
           ...
           "octalmode] \nConnect Virtual LAN n to PORT n of a vde switch "
           ...
           "option is available only if QEMU has been compiled with vde "
           "support enabled."));
  ...
  // -net dump[,file=file][,len=len]
    ...
    QMessageBox::information(
        this, tr("dump"),
        tr("-net dump[,file=f][,len=n] \ndump traffic on v lan \'n\' to file "
...
void Network_Widget::on_CB_Network_Type_currentIndexChanged(int index) {
  ...
  // -net dump[,file=file][,len=len]
    ...
    if (ui.CB_Network_Type->count() > 0)
      AQError("void Network_Widget::on_CB_Network_Type_currentIndexChanged( "
              "int index )",
              "Invalid connection type! Data: " +
                  ui.CB_Network_Type->currentText());
  }
...
void Network_Widget::Set_Net_Card_To_Ui(const VM_Net_Card_Native &card) {
  ...
  default:
    AQError("void Network_Widget::Set_Net_Card_To_Ui( const VM_Net_Card_Native "
            "&card )",
            "Cannot Read Network Type! Use Default: User Mode");
    ui.CB_Network_Type->setCurrentIndex(1);
    break;
...
bool Network_Widget::Net_Card_is_Valid() {
  ...
  if (u_helper && ui.CH_helper->isChecked()) {
    QString helper = ui.Edit_helper->text();
    if (helper != "" && !QFile::exists(helper))
...
void Network_Widget::Update_Card_Item() {
  ...
  case 1:
    ui.Items_List->currentItem()->setText(
        "User " +
        (item_name.isEmpty() ? Get_Items_Count(VM::Net_Mode_Native_User) : n));
    break;

...
void Add_Port_Window::Show_Arg_Help() {
  ...
  argsHelp << tr("[remote_host]:remote_port[@[src_ip]:src_port]\nThis "
                 "implements UDP Net Console. When remote_host or src_ip are "
                 "not specified they default to 0.0.0.0. When not using a "
  ...
  argsHelp << tr("host:port[,server][,nowait][,nodelay]\nThe telnet protocol "
                 "is used instead of raw tcp sockets.");
  argsHelp << tr(
  ...
  argsHelp << tr("Three button serial mouse. Configure the guest to use "
                 "Microsoft protocol.");
  argsHelp << tr("mon:dev_string\nThis is a special option to allow the "
                 "monitor to be multiplexed onto another serial port.");

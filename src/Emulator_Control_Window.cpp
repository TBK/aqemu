...
void Emulator_Control_Window::Create_Connect_Menu() {
  ...
  for (int ix = 0; ix < devices.count() - 1; ++ix) {
    ...
    // ide0-hd0: removable=0 io-status=ok file=/tmp/vl.0x5urG
    // backing_file=/mnt/os/vm/winxp_empty.qcow2 ro=0 drv=qcow2 encrypted=0
...
void Emulator_Control_Window::Create_Device_Menu() {
  ...
  // Parse line
  QString internalDevName =
      QString(value).replace(QRegExp("\\:.*"), ""); // Get device name
...
void Emulator_Control_Window::Connect_Device() {
  ...
  if (act) {
    ...
    // emit Ready_Read_Command( QString("change %1
    // \"%2\"").arg(nameAndPath[0]).arg(nameAndPath[1]) );
  ...
  if (usb_item) {
    ...
    for (int ix = 0; ix < ac_list.count(); ++ix) {
      if (usb_item->data().toString() == ac_list[ix]->data().toString()) {
        ...
        QString bus_addr; // FIXME = Cur_VM->Get_USB_Bus_Address(
                          // usb_item->data().toString() );
...
bool Emulator_Control_Window::Use_VNC() {
  if (Settings.value("Use_VNC_Display", "no").toString() == "yes" &&
      ...
      Cur_VM->Get_Video_Card() != "none") // TODO: Should also work seemlessly
                                          // if VNC is set to use a socket
  {
    return true;
  } else {
    return false;

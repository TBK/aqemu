...
private:
  enum class Kind { Empty, Bool, Long, String, StringList };
...
namespace std {
template <> struct hash<docopt::value> {
...
inline value::value(std::string v) : kind(Kind::String) {
  new (&variant.strValue) std::string(std::move(v));
}

inline value::value(std::vector<std::string> v) : kind(Kind::StringList) {
  new (&variant.strList) std::vector<std::string>(std::move(v));
}

inline value::value(value const &other) : kind(other.kind) {
  switch (kind) {
  case Kind::String:
...
inline value::value(value &&other) noexcept : kind(other.kind) {
  switch (kind) {
  case Kind::String:
  ...
  case Kind::StringList:
    new (&variant.strList)
        std::vector<std::string>(std::move(other.variant.strList));
    break;
...
inline value &value::operator=(value const &other) {
...
inline bool operator!=(value const &v1, value const &v2) { return !(v1 == v2); }
} // namespace docopt

...
// Arguments passed by user were incorrect (ie, developer was good, user is
// wrong)
...
/// @param options_first  Whether options must precede all args (true), or if
/// args and options
///                can be arbitrarily mixed.
///
/// @throws DocoptLanguageError if the doc usage string had errors itself
/// @throws DocoptExitHelp if 'help' is true and the user has passed the
/// '--help' argument
/// @throws DocoptExitVersion if 'version' is true and the user has passed the
/// '--version' argument
/// @throws DocoptArgumentError if the user's argv did not match the usage
/// patterns
...
///
/// Calls 'docopt_parse' and will terminate the program if any of the exceptions
/// above occur:
...
///  * DocoptArgumentError - print error and usage string and terminate (with
///  exit code -1)

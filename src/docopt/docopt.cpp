...
bool Required::match(
    PatternList &left,
    std::vector<std::shared_ptr<LeafPattern>> &collected) const {
  auto l = left;
  auto c = collected;
...
bool LeafPattern::match(
  ...
  auto same_name = std::find_if(collected.begin(), collected.end(),
                                [&](std::shared_ptr<LeafPattern> const &p) {
                                  return p->name() == name();
                                });
  if (getValue().isLong()) {
    long val = 1;
    ...
    if (same_name == collected.end()) {
      ...
      std::vector<std::string> const &list =
          (**same_name).getValue().asStringList();
...
Option Option::parse(std::string const &option_description) {
  ...
  if (double_space != std::string::npos) {
    options_end =
        option_description.begin() + static_cast<std::ptrdiff_t>(double_space);
  }

  static const std::regex pattern{"(-{1,2})?(.*?)([,= ]|$)"};
  for (std::sregex_iterator i{option_description.begin(), options_end, pattern,
    ...
    if (match[3].length() == 0) { // [3] always matches.
      // Hit end of string. For some reason 'match_not_null' will let us match
      // empty at the end, and then we'll spin in an infinite loop. So, if we
      // hit an empty match, we know we must be at the end.
      break;
    }
  }
  ...
  if (argcount) {
    std::smatch match;
    if (std::regex_search(
            options_end, option_description.end(), match,
            std::regex{"\\[default: (.*)\\]", std::regex::icase})) {
      val = match[1].str();
    }
  }

  return {std::move(shortOption), std::move(longOption), argcount,
          std::move(val)};
}

bool OneOrMore::match(
    PatternList &left,
    std::vector<std::shared_ptr<LeafPattern>> &collected) const {
...
bool Either::match(PatternList &left,
                   std::vector<std::shared_ptr<LeafPattern>> &collected) const {
  using Outcome =
      std::pair<PatternList, std::vector<std::shared_ptr<LeafPattern>>>;
  ...
  auto min = std::min_element(outcomes.begin(), outcomes.end(),
...
std::pair<size_t, std::shared_ptr<LeafPattern>>
Argument::single_match(PatternList const &left) const {
...
std::pair<size_t, std::shared_ptr<LeafPattern>>
Command::single_match(PatternList const &left) const {
  std::pair<size_t, std::shared_ptr<LeafPattern>> ret{};
...
std::pair<size_t, std::shared_ptr<LeafPattern>>
Option::single_match(PatternList const &left) const {
  std::pair<size_t, std::shared_ptr<LeafPattern>> ret{};
...
void BranchPattern::fix_repeating_arguments() {
  ...
  for (auto const &group : either) {
    ...
    std::unordered_multiset<std::shared_ptr<Pattern>, PatternHasher> group_set{
  ...
  while (!groups.empty()) {
    ...
    auto child_iter =
        std::find_if(children.begin(), children.end(),
                     [](std::shared_ptr<Pattern> const &p) {
...
public:
  Tokens(std::vector<std::string> tokens, bool isParsingArgv = true)
      : fTokens(std::move(tokens)), fIsParsingArgv(isParsingArgv) {}
  ...
  static Tokens from_pattern(std::string const &source) {
    static const std::regex re_separators{
        ...
        "("
        "[\\[\\]\\(\\)\\|]" // one character of brackets or parens or pipe
                            // character
        "|"
        "\\.\\.\\." // elipsis
        ")"};
    ...
    // We do two stages of regex matching. The '[]()' and '...' are strong
    // delimeters and need to be split out anywhere they occur (even at the end
    // of a token). We first split on those, and then parse the stuff between
    // them to find the string tokens. This is a little harder than the python
    // version, since they have regex.split and we dont have anything like that.

    std::vector<std::string> tokens;
    std::for_each(
        std::sregex_iterator{source.begin(), source.end(), re_separators},
        std::sregex_iterator{}, [&](std::smatch const &match) {
          // handle anything before the separator (this is the "stuff" between
          // the delimeters)
          if (match.prefix().matched) {
            std::for_each(
                std::sregex_iterator{match.prefix().first,
                                     match.prefix().second, re_strings},
                std::sregex_iterator{},
  ...
  std::string pop() { return std::move(fTokens.at(fIndex++)); }
...
template <typename T> std::vector<T *> flat_filter(Pattern &pattern) {
  ...
  std::vector<T *> ret;
  std::transform(flattened.begin(), flattened.end(), std::back_inserter(ret),
                 [](Pattern *p) { return static_cast<T *>(p); });
  return ret;
}

static std::vector<std::string> parse_section(std::string const &name,
                                              std::string const &source) {
  // ECMAScript regex only has "?=" for a non-matching lookahead. In order to
  // make sure we always have a newline to anchor our matching, we have to avoid
  // matching the final newline of each grouping. Therefore, our regex is
  // adjusted from the docopt Python one to use ?= to match the newlines before
  ...
  std::regex const re_section_pattern{
      ...
      "[^\\n]*" +
          ...
          "(?:\\n[ \\t].*?(?=\\n|$))*" // followed by any number of lines that
                                       // are indented
          ")",
      std::regex::icase};
  ...
  std::for_each(
      std::sregex_iterator(source.begin(), source.end(), re_section_pattern),
      std::sregex_iterator(),
...
template <typename I> std::vector<std::string> longOptions(I iter, I end) {
  std::vector<std::string> ret;
...
static PatternList parse_long(Tokens &tokens, std::vector<Option> &options) {
  ...
  if (similar.size() > 1) { // might be simply specified ambiguously 2+ times?
    std::vector<std::string> prefixes =
        longOptions(similar.begin(), similar.end());
...
static Required parse_pattern(std::string const &source,
...
static PatternList parse_argv(Tokens tokens, std::vector<Option> &options,
                              bool options_first) {
...
static std::vector<Option> parse_defaults(std::string const &doc) {
  // This pattern is a bit more complex than the python docopt one due to lack
  // of re.split. Effectively, it grabs any line with leading whitespace and
  // then a hyphen; it stops grabbing when it hits another line that also looks
  // like that.
  static std::regex const pattern{
      "(?:^|\\n)[ \\t]*"  // a new line with leading whitespace
      "(-(.|\\n)*?)"      // a hyphen, and then grab everything it can...
      "(?=\\n[ \\t]*-|$)" //  .. until it hits another new line with space and a
                          //  hyphen
  };
  ...
  for (auto s : parse_section("options:", doc)) {
    s.erase(s.begin(), s.begin() + static_cast<std::ptrdiff_t>(s.find(':')) +
                           1); // get rid of "options:"

    std::for_each(std::sregex_iterator{s.begin(), s.end(), pattern},
                  std::sregex_iterator{}, [&](std::smatch const &m) {
...
static bool isOptionSet(PatternList const &options, std::string const &opt1,
                        std::string const &opt2 = "") {
  return std::any_of(options.begin(), options.end(),
                     [&](std::shared_ptr<Pattern const> const &opt) -> bool {
...
static std::pair<Required, std::vector<Option>>
create_pattern_tree(std::string const &doc) {
  ...
  std::vector<Option const *> pattern_options =
      flat_filter<Option const>(pattern);

  using UniqueOptions =
      std::unordered_set<Option const *, PatternHasher, PatternPointerEquality>;
  UniqueOptions const uniq_pattern_options{pattern_options.begin(),
                                           pattern_options.end()};
...
docopt::docopt_parse(std::string const &doc,
                     std::vector<std::string> const &argv, bool help,
                     bool version, bool options_first) {
  Required pattern;
  std::vector<Option> options;
  try {
  ...
  throw DocoptArgumentError(
      "Arguments did not match expected patterns"); // BLEH. Bad error.
}

std::map<std::string, value>
docopt::docopt(std::string const &doc, std::vector<std::string> const &argv,
               bool help, std::string const &version,

...
bool Virtual_Machine::Create_VM_File(const QString &file_name,
  ...
  if (file_name.isEmpty()) {
    AQError("bool Virtual_Machine::Create_VM_File( const QString &file_name, "
            "bool template_mode )",
            "File Name is Empty!");
    return false;
  }
  ...
  for (int bx = 0; bx < Boot_Order_List.count(); bx++) {
    ...
    default:
      AQWarning("bool Virtual_Machine::Create_VM_File( const QString "
                "&file_name, bool template_mode )",
...
bool Virtual_Machine::Load_VM(const QString &file_name) {
  ...
  if (!VM_File.open(QFile::ReadOnly | QFile::Text)) {
    ...
    if (!VM_Dom_Document.setContent(&VM_File, true, &Error_String, &Error_Line,
      ...
      // Emulator Found?
      /*if( Current_Emulator.Get_Name().isEmpty() &&
      ...
      if (!Icon_Path.startsWith(":/") && !QFile::exists(Icon_Path)) {
...
VM_Native_Storage_Device Virtual_Machine::Load_VM_Native_Storage_Device(
  ...
  if (interface_str == "IDE")
    ...
    AQError("VM_Native_Storage_Device "
            "Virtual_Machine::Load_VM_Native_Storage_Device( const "
            "QDomElement &Second_Element ) const",
            "Storage Device Interface Default Section! Value: \"" +
                interface_str + "\"");
  }
  ...
  if ((Use_Native_Network() == false && Network_Cards.count() < 1) ||
    ...
    if (Use_Native_Network()) {
      for (int nc = 0; nc < Network_Cards_Native.count(); nc++) {
        QString nic_str = "";
        bool u_macaddr, u_model, u_name, u_hostname, u_port_dev, u_fd, u_ifname,
            u_script, u_downscript, u_listen, u_connect, u_mcast, u_sock,
            u_port, u_group, u_mode, u_file, u_len, u_addr, u_vectors, u_net,
            u_host, u_restrict, u_dhcpstart, u_dns, u_tftp, u_bootfile,
            u_hostfwd, u_guestfwd, u_smb, u_smbserver, u_sndbuf, u_vnet_hdr,
            u_vhost, u_vhostfd;
        bool u_bridge, u_helper;

        u_macaddr = u_model = u_name = u_hostname = u_port_dev = u_fd =
            u_ifname = u_script = u_downscript = u_listen = u_connect =
                u_mcast = u_sock = u_port = u_group = u_mode = u_file = u_len =
                    u_addr = u_vectors = u_net = u_host = u_restrict =
                        u_dhcpstart = u_dns = u_tftp = u_bootfile = u_hostfwd =
                            u_guestfwd = u_smb = u_smbserver = u_sndbuf =
                                u_vnet_hdr = u_vhost = u_vhostfd = false;
        u_bridge = u_helper = false;
        ...
        case VM::Net_Mode_Native_User:
          nic_str += "user";
          u_name = u_net = u_host = u_restrict = u_hostname = u_dhcpstart =
              u_dns = u_tftp = u_bootfile = u_hostfwd = u_guestfwd = u_smb =
                  u_smbserver = true;
          break;
        ...
        // -net bridge[,name=str][,br=bridge][,helper=helper]
        case VM::Net_Mode_Native_Bridge:
          nic_str += "bridge";
        ...
        case VM::Net_Mode_Native_TAP:
          nic_str += "tap";
          u_name = u_fd = u_ifname = u_script = u_downscript = u_sndbuf =
        ...
        if (Network_Cards_Native[nc].Use_Restrict() && u_restrict &&
          ...
          nic_str += ",restrict=" + Network_Cards_Native[nc].Get_Restrict()
                         ? "y"
                         : "n";
        ...
        if (Network_Cards_Native[nc].Use_VNet_hdr() && u_vnet_hdr &&
          ...
          nic_str += ",vnet_hdr=" + Network_Cards_Native[nc].Get_VNet_hdr()
                         ? "on"
                         : "off";
        ...
        if (Network_Cards_Native[nc].Get_VHost() && u_vhost &&
          ...
          nic_str +=
              ",vhost=" + Network_Cards_Native[nc].Get_VHost() ? "on" : "off";
...
QStringList Virtual_Machine::Build_Native_Device_Args(
  ...
  if (device.Use_Interface()) {
    ...
    default:
      AQError("QStringList Virtual_Machine::Build_Native_Device_Args( "
...
QStringList Virtual_Machine::Build_Shared_Folder_Args(
  ...
  //    -virtfs fsdriver,id=[id],path=[path to
  //    share],security_model=[mapped|passthrough|none][,writeout=writeout][,readonly]
  ...
  if ((Current_Emulator_Devices.PSO_KVM ||
    ...
    if (!lsmod.waitForFinished(1000)) {
      ...
      if (all_mod.isEmpty()) {
        ...
        if (kvm_intel_mod.exactMatch(all_mod))
          kvm_ok = true;
        else if (kvm_amd_mod.exactMatch(all_mod))
          kvm_ok = true;
        ...
        if (!kvm_ok) {
          int retVal = QMessageBox::question(
              ...
              tr("KVM Kernel Module Not Loaded!\n"
                 "To load this Module, Enter in Terminal with root "
                 "privileges: \"modprobe kvm-intel\". "
...
void Virtual_Machine::Save_VM_State() { Save_VM_State("aqemu_save", true); }
...
const QString &Virtual_Machine::Get_UID() const { return UID; }

void Virtual_Machine::Set_UID(const QString &uid) { UID = uid; }
...
void Virtual_Machine::Show_Monitor_Window() { Emu_Ctl->Show_Monitor(); }
...
bool Virtual_Machine::Take_Screenshot(const QString &file_name, int width,
  ...
  if (!(State == VM::VMS_Running || State == VM::VMS_Pause)) {
    AQError("bool Virtual_Machine::Take_Screenshot( const QString "
...
void Virtual_Machine::Update_Current_Emulator_Devices() {
  ...
  if (Current_Emulator.Get_Force_Version() ||
    ...
    case VM::QEMU_2_0:
      ...
      /*
      case VM::KVM_2_0:
          Current_Emulator_Devices = System_Info::Emulator_KVM_2_0[
      Computer_Type ]; break; */ //tobgle //FIXME?
...
const Available_Devices *Virtual_Machine::Get_Current_Emulator_Devices() const {
  if (Current_Emulator_Devices.System.QEMU_Name.isEmpty()) {
    AQError("const Virtual_Machine::Available_Devices "
...
QString
Virtual_Machine::Get_Current_Emulator_Binary_Path(const QString &names) const {
  QMap<QString, QString> bin_list = Current_Emulator.Get_Binary_Files();
  QStringList nl = names.split(" ", QString::SkipEmptyParts);

  if (bin_list.count() <= 0 || nl.count() <= 0) {
    AQError("QString Virtual_Machine::Get_Current_Emulator_Binary_Path( "
...
VM::VM_State Virtual_Machine::Get_State() const { return State; }
...
const QString &Virtual_Machine::Get_Icon_Path() const { return Icon_Path; }
...
int Virtual_Machine::Get_Memory_Size() const { return Memory_Size; }
...
const QString &Virtual_Machine::Get_CPU_Type() const { return CPU_Type; }

void Virtual_Machine::Set_CPU_Type(const QString &type) { CPU_Type = type; }

VM::SMP_Options Virtual_Machine::Get_SMP() const { return SMP; }

void Virtual_Machine::Set_SMP(const VM::SMP_Options &smp) { SMP = smp; }

int Virtual_Machine::Get_SMP_CPU_Count() const { return SMP.SMP_Count; }

void Virtual_Machine::Set_SMP_CPU_Count(int count) { SMP.SMP_Count = count; }
...
VM::Sound_Cards Virtual_Machine::Get_Audio_Cards() const { return Audio_Card; }

void Virtual_Machine::Set_Audio_Cards(VM::Sound_Cards card) {
  Audio_Card = card;
}

const QString &Virtual_Machine::Get_Video_Card() const { return Video_Card; }

void Virtual_Machine::Set_Video_Card(const QString &card) { Video_Card = card; }
...
bool Virtual_Machine::Get_Only_User_Args() const { return Only_User_Args; }

void Virtual_Machine::Set_Only_User_Args(bool use) { Only_User_Args = use; }
...
bool Virtual_Machine::Get_Show_Boot_Menu() const { return Show_Boot_Menu; }

void Virtual_Machine::Set_Show_Boot_Menu(bool use) { Show_Boot_Menu = use; }

bool Virtual_Machine::Use_Fullscreen_Mode() const { return Fullscreen; }

void Virtual_Machine::Use_Fullscreen_Mode(bool use) { Fullscreen = use; }

bool Virtual_Machine::Use_ACPI() const { return ACPI; }

void Virtual_Machine::Use_ACPI(bool use) { ACPI = use; }

bool Virtual_Machine::Use_Local_Time() const { return Local_Time; }

void Virtual_Machine::Use_Local_Time(bool use) { Local_Time = use; }

bool Virtual_Machine::Use_Snapshot_Mode() const { return Snapshot_Mode; }

void Virtual_Machine::Use_Snapshot_Mode(bool use) { Snapshot_Mode = use; }

bool Virtual_Machine::Use_Win2K_Hack() const { return Win2K_Hack; }

void Virtual_Machine::Use_Win2K_Hack(bool use) { Win2K_Hack = use; }

bool Virtual_Machine::Use_Start_CPU() const { return Start_CPU; }

void Virtual_Machine::Use_Start_CPU(bool start) { Start_CPU = start; }

bool Virtual_Machine::Use_No_Reboot() const { return No_Reboot; }

void Virtual_Machine::Use_No_Reboot(bool use) { No_Reboot = use; }

bool Virtual_Machine::Use_No_Shutdown() const { return No_Shutdown; }

void Virtual_Machine::Use_No_Shutdown(bool use) { No_Shutdown = use; }
...
const VM_Storage_Device &Virtual_Machine::Get_FD0() const { return FD0; }

void Virtual_Machine::Set_FD0(const VM_Storage_Device &floppy) { FD0 = floppy; }

const VM_Storage_Device &Virtual_Machine::Get_FD1() const { return FD1; }

void Virtual_Machine::Set_FD1(const VM_Storage_Device &floppy) { FD1 = floppy; }

const VM_Storage_Device &Virtual_Machine::Get_CD_ROM() const { return CD_ROM; }

void Virtual_Machine::Set_CD_ROM(const VM_Storage_Device &cdrom) {
  CD_ROM = cdrom;
}

const VM_HDD &Virtual_Machine::Get_HDA() const { return HDA; }

void Virtual_Machine::Set_HDA(const VM_HDD &hdd) { HDA = hdd; }

const VM_HDD &Virtual_Machine::Get_HDB() const { return HDB; }

void Virtual_Machine::Set_HDB(const VM_HDD &hdd) { HDB = hdd; }

const VM_HDD &Virtual_Machine::Get_HDC() const { return HDC; }

void Virtual_Machine::Set_HDC(const VM_HDD &hdd) { HDC = hdd; }

const VM_HDD &Virtual_Machine::Get_HDD() const { return HDD; }

void Virtual_Machine::Set_HDD(const VM_HDD &hdd) { HDD = hdd; }
...
void Virtual_Machine::Set_Snapshot(int index, const VM_Snapshot &s) {
  if (index >= 0 && index < Snapshots.count()) {
    ...
    AQError("void Virtual_Machine::Set_Snapshot( int index, const "
            "VM_Snapshot &s )",
...
const QList<VM_Shared_Folder> &
Virtual_Machine::Get_Shared_Folders_List() const {
...
const VM_Native_Storage_Device &
Virtual_Machine::Get_Storage_Device(int index) const {
...
void Virtual_Machine::Set_Storage_Device(
    int index, const VM_Native_Storage_Device &device) {
  if (index < 0 || index > Storage_Devices.count()) {
    // FIXME message
    return;
  }
...
const QString &Virtual_Machine::Get_TFTP_Prefix() const { return TFTP_Prefix; }

void Virtual_Machine::Set_TFTP_Prefix(const QString &p) { TFTP_Prefix = p; }
...
const QList<VM_Net_Card_Native> &
Virtual_Machine::Get_Network_Cards_Native() const {
  return Network_Cards_Native;
}

void Virtual_Machine::Set_Network_Cards_Nativ(
...
void Virtual_Machine::Set_VM_Network_Card(int index, const VM_Net_Card &nc) {
  if (index >= 0 && index <= 8 && index < Network_Cards.count()) {
    ...
    AQError("void Virtual_Machine::Set_VM_Network_Card( int index, "
            "VM_Net_Card *nc )",
            "Invalid Index!");
  }
}
...
void Virtual_Machine::Clear_Network_Cards_List() { Network_Cards.clear(); }

int Virtual_Machine::Get_Network_Cards_Count() const {
  return Network_Cards.count();
}

const VM_Redirection &
Virtual_Machine::Get_Network_Redirection(int index) const {
  if (index >= 0 && index < Network_Redirections.count()) {
    return Network_Redirections[index];
  } else {
    AQError("VM_Redirection *Virtual_Machine::Get_Network_Redirection( int "
            "index ) const",
            "Invalid Index!");
...
void Virtual_Machine::Set_Network_Redirection(int index,
  ...
  if (index >= 0 && index < Network_Redirections.count()) {
    ...
    AQError("void Virtual_Machine::Set_Network_Redirection( int index, "
            "VM_Redirection *r )",
            "Invalid Index!");
  }
}
...
bool Virtual_Machine::Get_Use_Network() const { return Use_Network; }

void Virtual_Machine::Set_Use_Network(bool use) { Use_Network = use; }

bool Virtual_Machine::Use_Native_Network() const { return Native_Network; }

void Virtual_Machine::Use_Native_Network(bool use) { Native_Network = use; }

bool Virtual_Machine::Get_Use_Redirections() const { return Use_Redirections; }

void Virtual_Machine::Set_Use_Redirections(bool r) { Use_Redirections = r; }
...
QString Virtual_Machine::Get_USB_Bus_Address( const QString &id )
{
    ...
    {
        AQError( "QString Virtual_Machine::Get_USB_Bus_Address( const QString
&id )", "info_usbhost_res.isEmpty()" ); return "";
    }
...
QString::SkipEmptyParts );
    ...
    {
        AQError( "QString Virtual_Machine::Get_USB_Bus_Address( const QString
&id )", "usbhost_dev_list.count() <= 0" ); return "";
    }

    // Valid String is: "Class 00: USB device ab12:34cd, USB UPS"
    QRegExp usbhost_dev_reg = QRegExp(
".+([\\da-fA-F]{,4}:[\\da-fA-F]{,4}),\\s+(.+)" );
    ...
    for( int ix = 0; ix < usbhost_dev_list.count(); ++ix )
    {
        ...
        {
            AQError( "QString Virtual_Machine::Get_USB_Bus_Address( const
QString &id )", "info_lines.count() <= 1" ); return "";
        }
...
&id )", "dev_name.isEmpty()" ); return "";
    ...
    {
        AQError( "QString Virtual_Machine::Get_USB_Bus_Address( const QString
&id )", "info_usb_res is Empty" ); return "";
    }
...
QString::SkipEmptyParts );
    ...
    {
        AQError( "QString Virtual_Machine::Get_USB_Bus_Address( const QString
&id )", "usb_dev_list.count() <= 0" ); return "";
    }

    // Valid String is: "Device 0.0, Speed 1.5 Mb/s, Product USB UPS"
    QRegExp usb_dev_reg = QRegExp(
"\\s*Device\\s+([\\d]+[.][\\d]+),\\s+Speed\\s+[\\d]+[.][\\d]+\\sMb/s,\\sProduct\\s+(.+)"
);
...
bool Virtual_Machine::Get_Use_Linux_Boot() const { return Linux_Boot; }

void Virtual_Machine::Set_Use_Linux_Boot(bool use) { Linux_Boot = use; }
...
const QString &Virtual_Machine::Get_Initrd_Path() const { return Initrd_Path; }
...
bool Virtual_Machine::Get_Use_ROM_File() const { return Use_ROM_File; }

void Virtual_Machine::Set_Use_ROM_File(bool use) { Use_ROM_File = use; }

const QString &Virtual_Machine::Get_ROM_File() const { return ROM_File; }

void Virtual_Machine::Set_ROM_File(const QString &path) { ROM_File = path; }

bool Virtual_Machine::Use_MTDBlock_File() const { return MTDBlock; }

void Virtual_Machine::Use_MTDBlock_File(bool use) { MTDBlock = use; }
...
bool Virtual_Machine::Use_SecureDigital_File() const { return SecureDigital; }

void Virtual_Machine::Use_SecureDigital_File(bool use) { SecureDigital = use; }
...
bool Virtual_Machine::Use_PFlash_File() const { return PFlash; }

void Virtual_Machine::Use_PFlash_File(bool use) { PFlash = use; }

const QString &Virtual_Machine::Get_PFlash_File() const { return PFlash_File; }

void Virtual_Machine::Set_PFlash_File(const QString &file) {
  PFlash_File = file;
}

bool Virtual_Machine::Use_KVM() const { return Enable_KVM; }

void Virtual_Machine::Use_KVM(bool use) { Enable_KVM = use; }

bool Virtual_Machine::Use_KVM_IRQChip() const { return KVM_IRQChip; }

void Virtual_Machine::Use_KVM_IRQChip(bool use) { KVM_IRQChip = use; }

bool Virtual_Machine::Use_No_KVM_Pit() const { return No_KVM_Pit; }

void Virtual_Machine::Use_No_KVM_Pit(bool use) { No_KVM_Pit = use; }
...
bool Virtual_Machine::Use_KVM_Nesting() const { return KVM_Nesting; }

void Virtual_Machine::Use_KVM_Nesting(bool use) { KVM_Nesting = use; }
...
bool Virtual_Machine::Use_No_Frame() const { return No_Frame; }

void Virtual_Machine::Use_No_Frame(bool use) { No_Frame = use; }

bool Virtual_Machine::Use_Alt_Grab() const { return Alt_Grab; }

void Virtual_Machine::Use_Alt_Grab(bool use) { Alt_Grab = use; }

bool Virtual_Machine::Use_No_Quit() const { return No_Quit; }

void Virtual_Machine::Use_No_Quit(bool use) { No_Quit = use; }

bool Virtual_Machine::Use_Portrait() const { return Portrait; }

void Virtual_Machine::Use_Portrait(bool use) { Portrait = use; }

bool Virtual_Machine::Use_Show_Cursor() const { return Show_Cursor; }

void Virtual_Machine::Use_Show_Cursor(bool use) { Show_Cursor = use; }

bool Virtual_Machine::Use_Curses() const { return Curses; }

void Virtual_Machine::Use_Curses(bool use) { Curses = use; }

bool Virtual_Machine::Use_RTC_TD_Hack() const { return RTC_TD_Hack; }

void Virtual_Machine::Use_RTC_TD_Hack(bool use) { RTC_TD_Hack = use; }

bool Virtual_Machine::Use_Start_Date() const { return Start_Date; }

void Virtual_Machine::Use_Start_Date(bool use) { Start_Date = use; }
...
const VM_SPICE &Virtual_Machine::Get_SPICE() const { return SPICE; }

void Virtual_Machine::Set_SPICE(const VM_SPICE &spice) { SPICE = spice; }

bool Virtual_Machine::Use_VNC() const { return VNC; }

void Virtual_Machine::Use_VNC(bool use) { VNC = use; }

bool Virtual_Machine::Get_VNC_Socket_Mode() const { return VNC_Socket_Mode; }

void Virtual_Machine::Set_VNC_Socket_Mode(bool use) { VNC_Socket_Mode = use; }
...
bool Virtual_Machine::Use_VNC_Password() const { return VNC_Password; }

void Virtual_Machine::Use_VNC_Password(bool use) { VNC_Password = use; }

void Virtual_Machine::Set_VNC_Password(const QString &pas) {
  Execute_Emu_Ctl_Command(pas);
}

bool Virtual_Machine::Use_VNC_TLS() const { return VNC_TLS; }

void Virtual_Machine::Use_VNC_TLS(bool use) { VNC_TLS = use; }

bool Virtual_Machine::Use_VNC_x509() const { return VNC_x509; }

void Virtual_Machine::Use_VNC_x509(bool use) { VNC_x509 = use; }
...
bool Virtual_Machine::Use_VNC_x509verify() const { return VNC_x509verify; }

void Virtual_Machine::Use_VNC_x509verify(bool use) { VNC_x509verify = use; }
...
void Virtual_Machine::Parse_StdErr() {
  ...
  else
  {
      ...
      {
          QString cleanOutput = splitOutput.last().remove( QRegExp("\[[KD].")
  ); emit Clean_Console( cleanOutput.trimmed() ); emit Ready_StdErr(
  cleanOutput.simplified() ); Last_Output.append( convOutput.simplified() );
      }
  }
...
void Virtual_Machine::Send_Emulator_Command(const QString &text) {
...
#endif
  {
    if (Monitor_Socket->state() != QAbstractSocket::ConnectedState) {
      AQError("void Virtual_Machine::Send_Emulator_Command( const QString "
              "&text )",
...
void Virtual_Machine::QEMU_Finished(int exitCode,
  ...
  AQDebug("void Virtual_Machine::QEMU_Finished( int exitCode, "
          "QProcess::ExitStatus exitStatus )",
          "QEMU Finished");
...
void Virtual_Machine::Suspend_Finished(const QString &returned_text) {
  AQDebug("void Virtual_Machine::Suspend_Finished( const QString "
          "&returned_text )",
          QString("Data: \"%1\"").arg(returned_text));
  ...
  if (returned_text.simplified() == "(qemu)") {
    ...
    Hide_VM_Save_Window();
    AQGraphic_Warning(tr("Error!"), tr("You must add HDD image in QCOW2 "
                                       "format to save virtual machine!"));
  }
}
  ...
  if (Settings.value("Info/Show_Screenshot_in_Save_Mode", "no").toString() ==
    ...
    if (!QFile::exists(img_path)) {
      ...
      /*cursor.insertHtml( "<img width=\"" +
                         QString::number(width() - ui.Machines_List->width()
         -50) +
                         "\" src=\"" + img_path + "\">" );*/

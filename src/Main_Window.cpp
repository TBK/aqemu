...
Main_Window::Main_Window(QWidget *parent) : QMainWindow(parent) {
  ...
  //// code to sync sizes of widgets in Device Manager, Folder Sharing and Ports
  /// Tab Widget
...
void Main_Window::closeEvent(QCloseEvent *event) {
  ...
  /*// Find running VM
      for( int vx = 0; vx < VM_List.count(); ++vx )
      {
              ...
              {
                      int mes_res = QMessageBox::question( this, tr("Close
  AQEMU?"), tr("One or more VMs are running!\nTerminate all running VMs and
  close AQEMU?"), QMessageBox::Yes | QMessageBox::No, QMessageBox::No );
...
Virtual_Machine *Main_Window::Get_VM_By_UID(const QString &uid) {
  ...
  // VM Not Found!
  AQWarning("Virtual_Machine *Main_Window::Get_VM_By_UID( const QString &uid, "
            "bool &ok )",
            "UID Not Found!");
  return NULL;
}
...
const QMap<QString, Available_Devices>
Main_Window::Get_Devices_Info(bool *ok) const {
...
Available_Devices Main_Window::Get_Current_Machine_Devices(bool *ok) const {
  ...
  if (!devOk) {
    AQError("Available_Devices Main_Window::Get_Current_Machine_Devices( bool "
            "*ok ) const",
            "Cannot get devices!");
    *ok = false;
...
bool Main_Window::Create_VM_From_Ui(Virtual_Machine *tmp_vm,
  ...
  if (old_vm == NULL) {
    if (show_user_errors)
      AQError("bool Main_Window::Create_VM_From_Ui( Virtual_Machine *tmp_vm, "
              "Virtual_Machine *old_vm )",
              "old_vm == NULL");
  ...
  tmp_vm->Use_KVM_No_Pit_Reinjection(
  ui_kvm.CH_KVM_No_Pit_Reinjection->isChecked() );
...
bool Main_Window::Save_Virtual_Machines() { return true; }
...
// KVM_No_Pit_Reinjection
ui_kvm.CH_KVM_No_Pit_Reinjection->setChecked(
tmp_vm->Use_KVM_No_Pit_Reinjection() );
...
); //FIXME: deprecated //alternatives? else
ui_kvm.CH_No_KVM_IRQChip->setEnabled( false );

if( curComp.PSO_No_KVM_Pit ) ui_kvm.CH_No_KVM_Pit->setEnabled( true );
else ui_kvm.CH_No_KVM_Pit->setEnabled( false );

if( curComp.PSO_No_KVM_Pit_Reinjection )
ui_kvm.CH_KVM_No_Pit_Reinjection->setEnabled( true ); else
ui_kvm.CH_KVM_No_Pit_Reinjection->setEnabled( false );
...
QString Main_Window::Get_Storage_Device_Info_String(const QString &path) {
  ...
  if (!QFile::exists(path)) {
    AQWarning("QString Main_Window::Get_Storage_Device_Info_String( const "
              "QString &path )",
...
void Main_Window::VM_State_Changed(Virtual_Machine *vm, VM::VM_State s) {
  if (vm == NULL) {
    AQError("void Main_Window::VM_State_Changed( Virtual_Machine *vm, "
            "VM::VM_State s )",
            "Error: vm == NULL");
    return;
  }
...
// FIXME This will be rewritten in the future. Deleting and creating new
// tabs/layouts is not done optimally
...
void Main_Window::on_Machines_List_currentItemChanged(
  ...
  if (VM_List.count() < 1) {
    AQDebug("void Main_Window::on_Machines_List_currentItemChanged( "
            "QListWidgetItem* current, QListWidgetItem* previous )",
            "VM_List.count() < 1");
    return;
  }
  ...
  if (old_vm == NULL) {
    AQError("void Main_Window::on_Machines_List_currentItemChanged( "
            "QListWidgetItem *current, QListWidgetItem *previous )",
            "old_vm == NULL");
    return;
  }
  ...
  if (*old_vm != tmp_vm && old_vm->Get_State() != VM::VMS_In_Error &&
    ...
    AQWarning("void Main_Window::on_Machines_List_currentItemChanged( "
              "QListWidgetItem* current, QListWidgetItem* previous )",
...
void Main_Window::on_Machines_List_customContextMenuRequested(
    const QPoint &pos) {
...
void Main_Window::on_Machines_List_itemDoubleClicked(QListWidgetItem *item) {
  ...
  if (cur_vm == NULL) {
    AQError("void Main_Window::on_Machines_List_itemDoubleClicked( "
            "QListWidgetItem *item )",
            "cur_vm == NULL");
    return;
  }
...
bool Main_Window::No_Device_Found(const QString &name, const QString &path,
  ...
  if (retVal == QMessageBox::No)
    ...
    if (ui.Machines_List->currentRow() >= 0 &&
      ...
      if (cur_vm == NULL) {
        AQError("bool Main_Window::No_Device_Found( const QString &name, const "
...
void Main_Window::on_actionAbout_Qt_triggered() { QApplication::aboutQt(); }
...
void Main_Window::on_actionExit_triggered() { close(); }
...
void Main_Window::CB_Boot_Priority_currentIndexChanged(int index) {
  ...
  default:
    AQWarning("void Main_Window::on_CB_Boot_Priority_currentIndexChanged( int "
              "index )",
...
QString Main_Window::Copy_VM_Hard_Drive(const QString &vm_name,
                                        ...
                                        const VM_HDD &hd) {
  if (vm_name.isEmpty()) {
    AQError("QString Main_Window::Copy_VM_Hard_Drive( const QString &vm_name, "
    ...
    if (!QFile::copy(hd.Get_File_Name(), new_name)) {
      AQError("QString Main_Window::Copy_VM_Hard_Drive( const QString "
              "&vm_name, const QString &hd_name, const VM_HDD &hd )",
              "Copy Error!");
    }
...
QString Main_Window::Copy_VM_Floppy(const QString &vm_name,
  ...
  if (vm_name.isEmpty()) {
    AQError("QString Main_Window::Copy_VM_Floppy( const QString &vm_name, "
            "const QString &fd_name, const VM_Storage_Device &fd )",
            "vm_name is Empty!");
    return "";
  } else {
    QString new_name = QDir::toNativeSeparators(

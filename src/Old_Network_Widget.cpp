...
Old_Network_Widget::Old_Network_Widget(QWidget *parent) : QWidget(parent) {
  ...
  // IP Address
  QRegExp rx("^([1-9]|[1-9][\\d]|[1][\\d][\\d]|2[0-4][\\d]|25[0-5])(\\.([0-9]|["
...
bool Old_Network_Widget::Get_Network_Cards(QList<VM_Net_Card> &cards) {
  if (Network_Cards.count() < 1) {
    AQDebug("bool Old_Network_Widget::Get_Network_Cards( QList<VM_Net_Card> "
            "&cards )",
            "No Items");
    cards.clear();
    return true;
...
void Old_Network_Widget::on_Button_Add_Net_Card_clicked() {
  if (ui.Network_Cards_List->count() >= 8)
    ...
    new QListWidgetItem(tr("Card ") +
                            QString::number(ui.Network_Cards_List->count() + 1),
                        ui.Network_Cards_List);
...
void Old_Network_Widget::on_CB_Network_Card_Model_currentIndexChanged(
    int index) {
...
bool Old_Network_Widget::Net_Card_is_Valid(int index) {
  ...
  if (Network_Cards[index].Get_Net_Mode() == VM::Net_Mode_Tcplisten ||
    ...
    QRegExp rx_ip("^([1-9]|[1-9][\\d]|[1][\\d][\\d]|2[0-4][\\d]|25[0-5])(\\.(["
                  "0-9]|[1-9][\\d]|[1][\\d][\\d]|2[0-4][\\d]|25[0-5])){3}$");

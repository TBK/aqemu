...
public:
  ...
  public:
    ...
    Sound_Cards() {
      Audio_sb16 = Audio_es1370 = Audio_Adlib = Audio_PC_Speaker = Audio_GUS =
          Audio_AC97 = Audio_HDA = Audio_cs4231a = false;
    }

    bool isEnabled() {
      return (Audio_sb16 || Audio_es1370 || Audio_Adlib || Audio_PC_Speaker ||
              Audio_GUS || Audio_AC97 || Audio_HDA || Audio_cs4231a);
    }

    bool operator==(const Sound_Cards &v) const {
      if (Audio_sb16 == v.Audio_sb16 && Audio_es1370 == v.Audio_es1370 &&
          Audio_Adlib == v.Audio_Adlib &&
          Audio_PC_Speaker == v.Audio_PC_Speaker && Audio_GUS == v.Audio_GUS &&
          Audio_AC97 == v.Audio_AC97 && Audio_HDA == v.Audio_HDA &&
    ...
    bool operator!=(const Sound_Cards &v) const { return !operator==(v); }
  };
  ...
  public:
    ...
    bool operator==(const SMP_Options &v) const {
      if (SMP_Count == v.SMP_Count && SMP_Cores == v.SMP_Cores &&
          SMP_Threads == v.SMP_Threads && SMP_Sockets == v.SMP_Sockets &&
    ...
    bool operator!=(const SMP_Options &v) const { return !operator==(v); }
  };
  ...
  enum Network_Mode {
    ...
    Net_Mode_Tcpconnect,
    Net_Mode_Multicast,
    Net_Mode_Multicastfd,
    Net_Mode_Dummy
  };

  // New Network Modes
  enum Network_Mode_Nativ {
    Net_Mode_Native_NIC,
    Net_Mode_Native_User,
    Net_Mode_Native_Chanel,
    Net_Mode_Native_Bridge,
    Net_Mode_Native_TAP,
    Net_Mode_Native_Socket,
    Net_Mode_Native_MulticastSocket,
    Net_Mode_Native_VDE,
    Net_Mode_Native_Dump
  };

  // Image Compression Type (Used in SPICE)
  enum SPICE_Image_Compression_Type {
    SPICE_IC_Type_on,
    SPICE_IC_Type_auto_glz,
    SPICE_IC_Type_auto_lz,
    SPICE_IC_Type_quic,
    SPICE_IC_Type_glz,
    SPICE_IC_Type_lz,
    SPICE_IC_Type_off
  ...
  enum SPICE_Renderer {
    SPICE_Renderer_oglpbuf,
    SPICE_Renderer_oglpixmap,
    SPICE_Renderer_cairo
  };
};
...
public:
  ...
  VM_Storage_Device(bool enabled, const QString &file_name, bool Native_mode,
                    const VM_Native_Storage_Device
                        &device); // FIXME test const VM_Native_Storage_Device
                                  // &device IN OLD version without const
...
private:
  ...
  bool _Use_MAC, _Use_VLAN, _Use_Name, _Use_Hostname, _Use_File_Descriptor,
      _Use_Interface_Name, _Use_Bridge_Name, _Use_TUN_TAP_Script,
      _Use_TUN_TAP_Down_Script, _Use_Bridge_Helper, _Use_Listen, _Use_Connect,
      _Use_MCast, _Use_Sock, _Use_Port, _Use_Group, _Use_Mode, _Use_File,
      _Use_Len, _Use_Addr, _Use_Vectors, _Use_Net, _Use_Host, _Use_Restrict,
      _Use_DHCPstart, _Use_DNS, _Use_Tftp, _Use_Bootfile, _Use_HostFwd,
      _Use_GuestFwd, _Use_SMB, _Use_SMBserver, _Use_Sndbuf, _Use_VNet_hdr,
      _Use_VHost, _Use_VHostFd;
};
...
public:
  ...
  void Get_USB_QEMU_Devices(bool &keyboard, bool &mouse, bool &tablet,
                            bool &wacomTablet, bool &braille) const;
  void Set_USB_QEMU_Devices(bool keyboard, bool mouse, bool tablet,
